(() => {
  ...
  const BUILD = "v5.1.3-art-source-anchor";
  ...
  // Flexible bootstrap manifest. Future systems can swap/extend this list without changing renderer logic.
  const BOOT = {
    assetNamespace: "VEILBOUND_DOLL_ASSETS",
    assetParts: [
      "./doll-assets-v513-part1.js",
      "./doll-assets-v513-part2.js",
    ],
    version: "5.1.3-art-source-anchor",
  };
  ...
  const CFG = {
    ...
    playerBossAttackCooldown: 0.62,
    playerBossDamage: 20,
    bossHealth: 80,
    bossHitCooldown: 1.25,
    bossAttackCooldownMin: 1.85,
    bossAttackCooldownMax: 2.55,
    bossHealthDamage: 12,
    attackReach: 58,
    bossReach: 64,
  ...
  const SKINS = [
    { id: "anchor_doll", name: "Anchor Doll", swatch: "#f1bda3" },
    { id: "pink_witch", name: "Pink Witch", swatch: "#f472b6" },
  ];
  ...
  const Assets = {
    ...
    pending: 0,
    defs: {
      idle: { frames: 10, fps: 1.55, loop: true },
    ...
    rig: {
      // v5.1.3 art-source rule:
      // the current doll sheets are authored facing right, natural frame order.
      // runtime facing handles direction; no per-state mirror hacks here.
      idle:   { srcFlipX: false, frameReverse: false, y: 0 },
      walk:   { srcFlipX: false, frameReverse: false, y: 0 },
      attack: { srcFlipX: false, frameReverse: false, y: 0 },
      crawl:  { srcFlipX: false, frameReverse: false, y: 0 },
      jump:   { srcFlipX: false, frameReverse: false, y: 0 },
      fall:   { srcFlipX: false, frameReverse: false, y: 0 },
    },
    armorKeys: ["armor0_underwear", "armor1_stockings", "armor2_witch"],
    init() {
      ASSET_SRC = Object.assign({}, window[BOOT.assetNamespace] || {});
      Object.entries(ASSET_SRC).forEach(([key, src]) => {
        const anim = this.getAnimFromKey(key);
        if (!anim || !src) return;
        const def = this.defs[anim];
        this.defineSpriteSheet(key, { imageKey: key, frameW: CFG.spriteFrameW, frameH: CFG.spriteFrameH, ...def });
        this.loadImage(key, src);
      });
    },
    getAnimFromKey(key) {
      return Object.keys(this.defs).find(anim => key.endsWith(`_${anim}`)) || null;
    },
    loadImage(key, src) {
      const img = new Image();
      this.pending += 1;
    ...
    ensureSkinAssets() { return true; },
    getRig(anim) { return this.rig[anim] || this.rig.idle; },
    getLayerKeys(entity) {
      const anim = entity?.animation?.name || "idle";
      const armorKey = Animation.getArmorKey(entity);
      if (SkinManager.current().id === "pink_witch") {
        return [`doll_pink_witch_complete_${armorKey}_${anim}`];
      }
      return [`doll_anchor_body_${anim}`];
    },
  };
  ...
  const SkinManager = {
    ...
    loadSkinId() {
      try {
        const saved = localStorage.getItem("veilbound_skin_v513");
    ...
    set(id) {
      ...
      try { localStorage.setItem("veilbound_skin_v513", skin.id); } catch (_) {}
  ...
  const Animation = {
    ...
    getSheetForEntity(entity) {
      ...
      const anim = entity.animation.name || "idle";
      return Assets.getSpriteSheet(`doll_anchor_body_${anim}`) || Assets.getSpriteSheet("doll_anchor_body_idle");
    },
    getFrame(entity) {
      const sheet = this.getSheetForEntity(entity);
  ...
  const Field = {
    enter() {
      ...
      UI.levelMessage(`Build ${BUILD} — art-source anchor doll test`);
      UI.updateLevelHud();
      Screens.show("game");
    },
    update(dt) {
  ...
  const Boss = {
    ...
    update(dt) {
      ...
      if (b.defeated) {
        ...
        p.y = CFG.groundY - p.h;
        p.grounded = true;
        Animation.choosePlayerAnimation(p, 0);
        Animation.update(p, dt);
        UI.updateBossHud();
        return;
      }
    ...
    releaseAttack() {
      ...
      if (b.telegraphType === "glob") {
        ...
        b.projectiles.push({ x: b.x + b.w / 2, y: b.y + 50, w: 28, h: 28, vx: dir * 210, life: 2.8, ignoreUntilClear: false });
      }
      b.telegraphType = "";
    },
    ...
    finish() {
      ...
      if (State.bossPlayer) {
        State.bossPlayer.vx = 0;
        State.bossPlayer.vy = 0;
        State.bossPlayer.crouching = false;
        State.bossPlayer.recoveryTimer = 2.0;
      }
  ...
  const Renderer = {
    ...
    drawPlayer(ctx, p, cam, fallen=false) {
      if (!p) return;
      const anim = p.animation?.name || "idle";
      const baseSheet = Assets.getSpriteSheet(`doll_anchor_body_${anim}`) || Assets.getSpriteSheet("doll_anchor_body_idle");
      if (!baseSheet) return this.drawFallbackPlayer(ctx, p, cam, fallen);
      const rig = Assets.getRig(anim);
      ...
      const y = p.y + p.h + 7 + (rig.y || 0);
      let frameIndex = Math.max(0, Math.min(baseSheet.frames - 1, p.animation?.frame || 0));
      if (rig.frameReverse && baseSheet.frames > 1) frameIndex = baseSheet.frames - 1 - frameIndex;
      const layerKeys = Assets.getLayerKeys(p);
      const facingScale = p.visualFacing * (rig.srcFlipX ? -1 : 1);
      ctx.save();
      ctx.translate(x, y);
      ...
      layerKeys.forEach((key) => {
        const sheet = Assets.getSpriteSheet(key);
        const img = sheet ? Assets.getImage(sheet.imageKey) : null;
        if (!sheet || !img || !img.complete || img.naturalWidth === 0) return;
        const f = Math.max(0, Math.min(sheet.frames - 1, frameIndex));
        ctx.drawImage(img, f * sheet.frameW, 0, sheet.frameW, sheet.frameH, -visualW/2, -visualH, visualW, visualH);
      });
      ctx.restore();
    },
    drawFallbackPlayer(ctx, p, cam, fallen=false) {
      ...
      const skin = SkinManager.current(); const pal = skin.id === "pink_witch" ? { hair: [244,114,182], outfit: [249,168,212], dark: [17,24,39] } : { hair: [241,189,163], outfit: [241,189,163], dark: [87,63,63] };
      ctx.fillStyle=`rgb(${pal.dark[0]},${pal.dark[1]},${pal.dark[2]})`; ctx.fillRect(-18,-78,36,58); ctx.fillStyle=`rgb(${pal.outfit[0]},${pal.outfit[1]},${pal.outfit[2]})`; ctx.fillRect(-16,-72,32,46); ctx.fillStyle=C.skin; ctx.beginPath(); ctx.arc(0,-90,15,0,Math.PI*2); ctx.fill(); ctx.fillStyle=`rgb(${pal.hair[0]},${pal.hair[1]},${pal.hair[2]})`; ctx.beginPath(); ctx.ellipse(-2,-96,20,16,-.2,0,Math.PI*2); ctx.fill(); ctx.restore();
    },
  ...
  const BootUI = {
    setText(text) {
      document.querySelectorAll("[data-build-tag]").forEach(tag => tag.textContent = text);
      const menuP = document.querySelector("#menu p");
      if (menuP) menuP.textContent = text.includes("loading") ? "Loading doll rig assets..." : "Doll rig bootstrap build.";
    },
    warn(text) {
      const menuP = document.querySelector("#menu p");
      if (menuP) menuP.textContent = text;
      console.warn(text);
    },
  };
  ...
  const Bootstrap = {
    ...
    scriptUrl(src) {
      ...
      return `${src}${sep}v=${encodeURIComponent(BOOT.version)}`;
    },
    loadScript(src) {
      return new Promise((resolve, reject) => {
        if (document.querySelector(`script[data-veilbound-asset="${src}"]`)) return resolve();
        const script = document.createElement("script");
        script.src = this.scriptUrl(src);
        script.async = false;
        script.defer = false;
        script.dataset.veilboundAsset = src;
    ...
    async loadAssets() {
      window[BOOT.assetNamespace] = window[BOOT.assetNamespace] || {};
      for (const src of BOOT.assetParts) {
        BootUI.setText(`${BUILD} loading ${src.split("/").pop()}`);
        await this.loadScript(src);
      }
      ASSET_SRC = Object.assign({}, window[BOOT.assetNamespace] || {});
      const count = Object.keys(ASSET_SRC).length;
      if (!count) throw new Error("No doll assets loaded into " + BOOT.assetNamespace);
      BootUI.setText(BUILD);
      this.loaded = true;
    },
    async start() {
      try {
        BootUI.setText(`${BUILD} loading`);
        await this.loadAssets();
        Game.init();
      } catch (err) {
        BootUI.setText(BUILD);
        BootUI.warn(`Asset boot failed: ${err.message}. Check that all doll-assets files are uploaded beside index.html.`);
        throw err;
      }
    },
  };
