(() => {
  ...
  const BUILD = "v5.1.2-clean-anchor-doll";
  ...
  const BOOT = {
    ...
    assetParts: [
      "./doll-assets-v512-part1.js",
    ],
    version: "5.1.2-clean-anchor-doll",
  };
  ...
  const Assets = {
    ...
    rig: {
      // v5.1.2 canonical doll source rule:
      // every generated doll sheet faces right in natural frame order.
      // runtime flip is only controlled by player.visualFacing.
    ...
    getLayerKeys(entity) {
      ...
      const baseKey = `doll_anchor_body_${anim}`;
      if (SkinManager.current().id === "pink_witch") {
        return [baseKey, `doll_pink_witch_overlay_${armorKey}_${anim}`];
      }
      return [baseKey];
    },
  };
  ...
  const SkinManager = {
    ...
    loadSkinId() {
      try {
        const saved = localStorage.getItem("veilbound_skin_v512");
    ...
    set(id) {
      ...
      try { localStorage.setItem("veilbound_skin_v512", skin.id); } catch (_) {}
  ...
  const Field = {
    enter() {
      ...
      UI.levelMessage(`Build ${BUILD} — polished anchor doll test`);
      UI.updateLevelHud();
      Screens.show("game");
    },
    update(dt) {
