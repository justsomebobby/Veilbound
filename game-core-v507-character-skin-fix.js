(() => {
  ...
  const BUILD = "v5.0.7-character-skin-fix";
  ...
  const SKINS = [
    ...
    { id: "tooka_standard", name: "Tooka", swatch: "#eab308" },
    { id: "aka_standard", name: "Aka Standard", swatch: "#f97316" },
    { id: "aka_bodysuit", name: "Aka Suit", swatch: "#ef4444" },
    { id: "white_witch", name: "White Witch", swatch: "#93c5fd" },
    { id: "savor_magical", name: "Savor", swatch: "#facc15" },
    { id: "orange_witch", name: "Orange Witch", swatch: "#fb923c" },
  ];
  ...
  const Assets = {
    ...
    init() {
      Object.entries(ASSET_SRC).forEach(([key, src]) => {
        const parsed = this.parsePlayerAssetKey(key);
        if (!parsed || !src) return;
        this.defineSpriteSheet(key, {
          imageKey: key,
          frameW: CFG.spriteFrameW,
          frameH: CFG.spriteFrameH,
          ...this.defs[parsed.anim],
        });
        this.loadImage(key, src);
      });
    },
    parsePlayerAssetKey(key) {
      if (!key || !key.startsWith("player_")) return null;
      const anims = Object.keys(this.defs);
      for (const armorKey of this.armorKeys) {
        for (const anim of anims) {
          const suffix = `_${armorKey}_${anim}`;
          if (key.endsWith(suffix)) return { armorKey, anim };
        }
      }
      return null;
    },
    ...
    ensureSkinAssets(skinId) {
      // v5.0.7 uses prebuilt per-character sprite sheets in the flat asset files.
      const testKey = `player_${skinId}_armor2_witch_idle`;
      return !!this.getSpriteSheet(testKey);
    },
  ...
  const Renderer = {
    ...
    drawPlayer(ctx, p, cam, fallen=false) {
      ...
      const y = p.y + p.h + 7;
      const drawScaleX = p.visualFacing; // source sheets are normalized in the asset pass; do not globally mirror.
      ctx.save();
      ctx.translate(x, y);
