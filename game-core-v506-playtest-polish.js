(() => {
  ...
  const BUILD = "v5.0.6-playtest-polish";
  ...
  const SKINS = [
    { id: "pink_witch", name: "Pink Witch", swatch: "#f472b6", palette: { hair: [244,114,182], outfit: [249,168,212], dark: [17,24,39], boots: [31,41,55] } },
    { id: "blue_star", name: "Blue Star", swatch: "#60a5fa", palette: { hair: [235,246,255], outfit: [96,165,250], dark: [30,41,89], boots: [230,245,255] } },
    { id: "sun_twin", name: "Sun Twin", swatch: "#fb923c", palette: { hair: [249,115,22], outfit: [251,146,60], dark: [67,38,20], boots: [31,41,55] } },
    { id: "academy_blonde", name: "Academy", swatch: "#facc15", palette: { hair: [253,230,138], outfit: [229,231,235], dark: [30,58,138], boots: [31,41,55] } },
    { id: "dark_hoodie", name: "Dark Hoodie", swatch: "#374151", palette: { hair: [250,250,250], outfit: [55,65,81], dark: [15,23,42], boots: [17,24,39] } },
    { id: "crimson_pilot", name: "Crimson", swatch: "#ef4444", palette: { hair: [249,115,22], outfit: [239,68,68], dark: [17,24,39], boots: [153,27,27] } },
    { id: "moon_guardian", name: "Moon Guard", swatch: "#2563eb", palette: { hair: [254,240,138], outfit: [37,99,235], dark: [15,23,42], boots: [245,248,255] } },
    { id: "forest_casual", name: "Forest Casual", swatch: "#22c55e", palette: { hair: [253,230,138], outfit: [34,197,94], dark: [20,83,45], boots: [31,41,55] } },
  ];
  ...
  const Assets = {
    ...
    generatedSkins: {},
    defs: {
      idle: { frames: 10, fps: 1.65, loop: true },
    ...
    init() {
      this.armorKeys.forEach((armorKey) => {
        Object.entries(this.defs).forEach(([anim, def]) => {
      ...
      this.defineSkinAliases("pink_witch");
    },
    loadImage(key, src) {
      const img = new Image();
      this.pending += 1;
    ...
    defineSkinAliases(skinId) {
      this.armorKeys.forEach((armorKey) => {
        Object.entries(this.defs).forEach(([anim, def]) => {
          const baseKey = `player_${armorKey}_${anim}`;
          const skinKey = `player_${skinId}_${armorKey}_${anim}`;
          this.defineSpriteSheet(skinKey, { imageKey: baseKey, frameW: CFG.spriteFrameW, frameH: CFG.spriteFrameH, ...def });
        });
      });
      this.generatedSkins[skinId] = true;
    },
    ensureSkinAssets(skinId) {
      if (this.generatedSkins[skinId]) return true;
      const skin = SkinManager.get(skinId);
      if (!skin || !skin.palette) return false;
      if (skinId === "pink_witch") { this.defineSkinAliases(skinId); return true; }
      let ready = true;
      this.armorKeys.forEach((armorKey) => {
        Object.entries(this.defs).forEach(([anim, def]) => {
          const baseKey = `player_${armorKey}_${anim}`;
          const baseImg = this.getImage(baseKey);
          if (!baseImg || !baseImg.complete || baseImg.naturalWidth === 0) { ready = false; return; }
          const imageKey = `player_${skinId}_${armorKey}_${anim}`;
          if (!this.images[imageKey]) this.images[imageKey] = this.makeReskinImage(baseImg, skin, armorKey);
          this.defineSpriteSheet(imageKey, { imageKey, frameW: CFG.spriteFrameW, frameH: CFG.spriteFrameH, ...def });
        });
      });
      if (ready) this.generatedSkins[skinId] = true;
      return ready;
    },
    makeReskinImage(baseImg, skin, armorKey) {
      ...
      for (let y = 0; y < canvas.height; y += 1) {
        ...
        for (let x = 0; x < canvas.width; x += 1) {
          ...
          const pink = r > 135 && b > 75 && g < 180 && r > g * 1.15 && (r - g) > 35;
          const dark = max < 112 && min < 92;
          if (pink) {
            const target = fy < 145 ? pal.hair : pal.outfit;
            this.tintPixel(p, i, r, g, b, target, 0.93);
          } else if (dark) {
            const target = fy > 228 ? pal.boots : pal.dark;
            this.tintPixel(p, i, r, g, b, target, 0.82);
          }
        }
      }
    ...
    tintPixel(p, i, r, g, b, target, strength) {
      const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
      const shade = 0.42 + lum * 0.88;
      ...
      p[i] = Math.round(r * (1 - strength) + tr * strength);
      p[i + 1] = Math.round(g * (1 - strength) + tg * strength);
      p[i + 2] = Math.round(b * (1 - strength) + tb * strength);
    },
  };
  ...
  const Animation = {
    ...
    getSheetForEntity(entity) {
      ...
      const skinId = SkinManager.current().id;
      Assets.ensureSkinAssets(skinId);
      const armorKey = this.getArmorKey(entity);
      const key = `player_${skinId}_${armorKey}_${anim}`;
      return Assets.getSpriteSheet(key)
        || Assets.getSpriteSheet(`player_${skinId}_${armorKey}_idle`)
        || Assets.getSpriteSheet(`player_${armorKey}_${anim}`)
        || Assets.getSpriteSheet(`player_${armorKey}_idle`);
    },
    getFrame(entity) {
      const sheet = this.getSheetForEntity(entity);
  ...
  const Renderer = {
    ...
    drawPlayer(ctx, p, cam, fallen=false) {
      ...
      const y = p.y + p.h + 7;
      const drawScaleX = -p.visualFacing; // cleaned source art faces left; mirror so gameplay facing is correct.
      ctx.save();
      ctx.translate(x, y);
    ...
    drawFallbackPlayer(ctx, p, cam, fallen=false) {
      ...
      const skin = SkinManager.current(); const pal = skin.palette || { hair: [244,114,182], outfit: [249,168,212], dark: [17,24,39] };
