(() => {
  ...
  const BUILD = "v5.0.9-anchor-doll-foundation";
  ...
  const Renderer = {
    ...
    drawPlayer(ctx, p, cam, fallen=false) {
      ...
      const y = p.y + p.h + 7;
      const drawScaleX = p.visualFacing; // source doll sheets face right; no global mirror.
      const frameIndex = Math.max(0, Math.min(baseSheet.frames - 1, p.animation?.frame || 0));
      const layerKeys = Assets.getDollLayerKeys(p);
      ctx.save();
      ctx.translate(x, y);
